...
/**
 * Configuration class for Coinbase Feign client
 */
@Configuration
@Slf4j
public class CoinbaseFeignConfig {
    ...
    /**
     * Configure request options for Coinbase API
     */
    @Bean("coinbaseRequestOptions")
     ...
     */
    @Bean("coinbaseFeignLoggerLevel")
    public Logger.Level feignLoggerLevel() {
        return Logger.Level.BASIC;
    }
    
    /**
     * Custom error decoder for handling Coinbase API errors
     */
    @Bean("coinbaseErrorDecoder")
    public ErrorDecoder errorDecoder() {
        return new CoinbaseErrorDecoder();
    }
    
    /**
     * Custom error decoder implementation for Coinbase API
     */
    public static class CoinbaseErrorDecoder implements ErrorDecoder {
        
        private final ErrorDecoder defaultErrorDecoder = new Default();
        
        @Override
        public Exception decode(String methodKey, feign.Response response) {
            log.error("Coinbase API error - Method: {}, Status: {}, Reason: {}", 
                     methodKey, response.status(), response.reason());
            
            // Handle specific Coinbase API error codes
