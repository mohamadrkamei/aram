...
/**
 * Configuration class for OKX Feign client
 */
@Configuration
@Slf4j
public class OKXFeignConfig {
    
    /**
     * Configure request options for OKX API
     */
    @Bean("okxRequestOptions")
     ...
     */
    @Bean("okxFeignLoggerLevel")
    public Logger.Level feignLoggerLevel() {
        return Logger.Level.BASIC;
    }
    
    /**
     * Custom error decoder for handling OKX API errors
     */
    @Bean("okxErrorDecoder")
    public ErrorDecoder errorDecoder() {
        return new OKXErrorDecoder();
    }
    
    /**
     * Custom error decoder implementation for OKX API
     */
    public static class OKXErrorDecoder implements ErrorDecoder {
        
        private final ErrorDecoder defaultErrorDecoder = new Default();
        
        @Override
        public Exception decode(String methodKey, feign.Response response) {
            log.error("OKX API error - Method: {}, Status: {}, Reason: {}", 
                     methodKey, response.status(), response.reason());
            
            // Handle specific OKX API error codes
